...
void testGetColumnGetRow(void) {
    ...
    c = getColumn(A, 1);
    printMatrix(c);
    printf("The 1st column\n\n");
    deleteMatrix(c);
    c = getColumn(A, 3);
    printMatrix(c);
    printf("The 3rd column\n\n");
    ...
    assert(!getColumn(A, 5));
    assert(!getRow(A, 5));
    assert(!getColumn(NULL, 2));
    assert(!getRow(NULL, 2));
    printf("Out of bounds and NULL return errors.\n\n");
...
void testScalarProduct(void) {
    ...
    printMatrix(A);
    printf("\n");
    err = scalarProduct(2, A);
    assert(!err);
    printMatrix(A);
    printf("times 2\n\n");

    err = scalarProduct(1.354, A);
    assert(!err);
    printMatrix(A);
    printf("times 1.354\n\n");

    assert(scalarProduct(2, NULL));
    printf("NULL returns an  error\n");
...
int symmetric(matrix const * mtx){
    if(mtx && mtx->rows == mtx->cols){
        int i, j, k;
        for(i=1,j=mtx->cols;i<j;i++,j--){
            for(k=1;k<=mtx->rows;k++){
                if(getE(mtx, k, i) != getE(mtx, k, j)) return 0;
            }
        }
    }else return 0;
    return 1;
...
matrix * getColumn(matrix const * mtx, int col){
    if(!mtx || col > mtx->cols) return NULL;
    int i;
    matrix * m;
    ...
    m->data = (double *) malloc(mtx->rows*sizeof(double));
    if (!m->data) {
        free(m);
        return NULL;
    }
    for (i = 1; i <= mtx->rows; i++)
        m->data[i-1] = getE(mtx, i, col);

    return m;
}

matrix * getRow(matrix const * mtx, int row){
    if(!mtx || row > mtx->rows) return NULL;
    int i;
    matrix * m;
    ...
    m->data = (double *) malloc(mtx->cols*sizeof(double));
    if (!m->data) {
        free(m);
        return NULL;
    }
    for (i = 1; i <= mtx->cols; i++)
        m->data[i-1] = getE(mtx, row, i);
...
int scalarProduct(double s, matrix * mtx){
    ...
    for(c=1;c<=mtx->cols;c++)
        for(r=1;r<=mtx->rows;r++) setE(mtx, r, c, s * getE(mtx, r, c));
    return 0;
}
